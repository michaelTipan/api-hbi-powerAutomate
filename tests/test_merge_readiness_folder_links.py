...
def _hist_bytes_two_pagos() -> bytes:
    ...
    from app.application.services.review_schema import DistribucionCols, ReviewSheets
...
def test_assess_pdf_258_in_folder_264_is_mismatch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Caso operador: PDF credito-258.pdf dentro de carpeta del crédito 264."""
    from io import BytesIO
    ...
    row, _ = make_distrib_row(
        ...
        cliente="Cliente 264",
        credito="264",
        ruta_pdf_internal="extractos/264.pdf",
    )
    ws.append(
        list(row)
    ...
    _patch_assess_common(monkeypatch, hist)

    async def _list_wrong(_g, _s, _d, path: str) -> list[dict[str, object]]:
        assert "264" in path
        return [
            {
                "name": "asiento_banco_bogota_credito-258.pdf",
                "file": {},
            }
        ]

    monkeypatch.setattr(mr, "_list_drive_folder_children", _list_wrong)

    result = asyncio.run(assess_merge_readiness(AsyncMock(), snap, "banco_bogota"))
    assert result.status == "incomplete"
    assert len(result.missing_items) == 1
    item = result.missing_items[0]
    assert item["error_code"] == "asiento_contable_credit_mismatch"
    assert item["credito"] == "264"
    assert item["found_pdf_name"] == "asiento_banco_bogota_credito-258.pdf"
    assert item["found_credit_hint"] == "258"
    assert "no coincide" in result.user_message.lower()
