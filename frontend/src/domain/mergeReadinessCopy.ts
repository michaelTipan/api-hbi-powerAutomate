...
export function buildMergeSupportOperationalIssues(
  ...
  return missingItems.map((item, index) => {
    ...
    return {
      ...
      value_found: item.found_pdf_name ?? item.found_credit_hint ?? null,
      expected_values: credito ? [credito] : [],
      next_action:
        "Corrija o renombre el PDF en SharePoint y use «Actualizar / verificar soportes».",
      retry: null,
      links,
